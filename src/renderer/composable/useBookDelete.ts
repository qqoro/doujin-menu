import * as api from "@/api";
import { usePermanentDelete } from "@/composable/usePermanentDelete";
import { useQueryClient } from "@tanstack/vue-query";
import { ref } from "vue";
import { toast } from "vue-sonner";
import type { Book } from "../../types/ipc";

/**
 * 라이브러리 삭제 다이얼로그의 페이지 레벨 상태.
 *
 * 카드가 각자 `AlertDialog`를 들고 있으면, 다이얼로그를 연 채 목록을 스크롤해
 * 그 카드가 언마운트될 때 다이얼로그까지 사라집니다. Reka UI가 잠그는 건 body
 * 스크롤인데 라이브러리의 스크롤러는 내부 div라 스크롤이 막히지 않기 때문입니다.
 * 가상 스크롤을 넣으면 언마운트가 훨씬 잦아 이 문제가 확실해집니다.
 *
 * 다이얼로그를 페이지에 하나만 두고, 카드는 `request-delete`만 올립니다.
 * 다운로더의 `useGalleryDelete`와 같은 구조입니다.
 */
export function useBookDelete() {
  const isOpen = ref(false);
  const target = ref<Book | null>(null);
  const queryClient = useQueryClient();

  // 영구 삭제 체크 상태 (모든 삭제 다이얼로그 공유, localStorage 유지)
  const { permanentDelete } = usePermanentDelete();

  const requestDelete = (book: Book) => {
    target.value = book;
    isOpen.value = true;
  };

  const confirmDelete = async () => {
    const book = target.value;
    if (!book) return;

    try {
      // 체크 상태에 따라 영구 삭제
      await api.deleteBook(book.id, { permanent: permanentDelete.value });
      toast.success("책 삭제 완료", {
        description: `${book.title}이(가) 삭제되었습니다.`,
      });
      queryClient.invalidateQueries({ queryKey: ["books"] });
    } catch (error) {
      console.error("책 삭제 실패:", error);
      toast.error("책 삭제 실패", {
        description:
          (error as Error).message || "책을 삭제하는 중 오류가 발생했습니다.",
      });
    } finally {
      isOpen.value = false;
    }
  };

  return { isOpen, target, permanentDelete, requestDelete, confirmDelete };
}
